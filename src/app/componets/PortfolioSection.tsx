...
const projects = [
    {
        ...
        beforeAlt: 'Outdated kitchen with worn cabinets, old appliances, and poor lighting before renovation',
        afterAlt: 'Stunning modern kitchen with white marble countertops, custom navy cabinetry, and gold fixtures after renovation',
    ...
    },
    {
        ...
        afterAlt: 'Luxurious spa-inspired master bathroom with rainfall shower and marble tiles after renovation',
        span: '',
        featured: false
    },
    {
        ...
        afterAlt: 'Bright open-concept living area with white walls, modern furniture, and abundant natural light after renovation',
        span: '',
        featured: false
    },
    {
        ...
        afterAlt: 'Modern boutique office with glass partitions, custom millwork, and designer lighting after renovation',
        span: '',
        featured: false
    },
    {
        ...
        afterAlt: 'Coastal-inspired kitchen with white shaker cabinets, quartz counters, and subway tile after renovation',
        span: '',
        featured: false
    }];


type FilterKey = 'All' | 'Todo' | 'Kitchen' | 'Cocinas' | 'Bathroom' | 'Baños' | 'Living Spaces' | 'Salas' | 'Commercial' | 'Comercial';

const filterMap: Record<string, string> = {
    'All': 'All', 'Todo': 'All',
    'Kitchen': 'Kitchen', 'Cocinas': 'Kitchen',
    'Bathroom': 'Bathroom', 'Baños': 'Bathroom',
    'Living Spaces': 'Living Spaces', 'Salas': 'Living Spaces',
...
export default function PortfolioSection({ lang }: PortfolioSectionProps) {
    ...
    const filtered = normalizedFilter === 'All' ?
        projects :
        projects.filter((p) => p.category === normalizedFilter);

    return (
        <section id="projects" ref={sectionRef} className="py-24 md:py-32 bg-secondary relative overflow-hidden">
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 reveal">
                    <div>
                        ...
                        <p className="text-lg text-muted-foreground mt-3 max-w-lg">{tx.sub}</p>
                    </div>
                    <a
                        href="#contact"
                ...
                <div className="flex flex-wrap gap-2 mb-10 reveal">
                    {tx.filters.map((filter) =>
                        <button
                            ...
                            className={`px-5 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${activeFilter === filter ?
          ...
          */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                    {filtered.map((project, idx) =>
                        <div
                            key={project.id}
                            className={`portfolio-card group relative rounded-3xl overflow-hidden bg-card border border-border cursor-pointer reveal ${project.featured && filtered.length > 2 ? 'lg:col-span-2 lg:row-span-2' : ''}`
                            }
                            style={{ transitionDelay: `${idx * 80}ms`, minHeight: project.featured && filtered.length > 2 ? '480px' : '280px' }}
                            ...
                            <div className="absolute top-4 left-4 flex gap-2">
                                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 ${hoveredId !== project.id ?
                                        'bg-black/50 text-white backdrop-blur-sm' : 'bg-accent text-accent-foreground'}`
                                }>
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-6">
                                <span className="text-xs uppercase tracking-widest text-accent font-semibold">{project.category}</span>
                                <h3 className="text-lg font-display font-bold text-white mt-1">{project.title}</h3>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <div className="flex flex-col items-center gap-2 text-white">
                                    ...
                                    <span className="text-xs font-medium">{tx.before} → {tx.after}</span>
                                </div>
                            </div>
                        </div>
                    )}
