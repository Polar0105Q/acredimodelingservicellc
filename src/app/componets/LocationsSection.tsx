...
const t = {
    en: {
        ...
        areas: [
            'Miami / Brickell', 'Coral Gables', 'Miami Beach', 'Coconut Grove',
            'Wynwood', 'Key Biscayne', 'Aventura', 'Fort Lauderdale',
            'Doral', 'Kendall', 'Homestead', 'Palm Beach',
        ],
        office: 'Main Office',
...
export default function LocationsSection({ lang }: LocationsSectionProps) {
    ...
    return (
        <section id="locations" ref={sectionRef} className="py-24 md:py-32 bg-background relative overflow-hidden">
                    ...
                    <div className="lg:col-span-2 relative rounded-3xl overflow-hidden min-h-[400px] reveal">
                        ...
                        <div className="absolute bottom-4 left-4 right-4 glass-card rounded-2xl p-4 flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl bg-accent flex items-center justify-center">
                                <Icon name="MapPinIcon" size={16} variant="solid" className="text-accent-foreground" />
                            </div>
                            <div>
                                <span className="text-xs font-bold text-white uppercase tracking-wider">South Florida Service Area</span>
                                <span className="block text-xs text-white/60">Miami-Dade · Broward · Palm Beach</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-5 reveal">
                        ...
                        <div className="bg-card border border-border rounded-2xl p-6">
                            ...
                            <div className="space-y-2">
                                <a href={`tel:${tx.phone}`} className="flex items-center gap-2 text-sm text-foreground hover:text-primary transition-colors">
                                    <Icon name="PhoneIcon" size={14} variant="outline" className="text-accent" />
                                    {tx.phone}
                                </a>
                                <a href={`mailto:${tx.email}`} className="flex items-center gap-2 text-sm text-foreground hover:text-primary transition-colors">
                        ...
                        <div className="bg-card border border-border rounded-2xl p-6">
                            <div className="flex items-center gap-3 mb-4">
                                <div className="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center">
                                    <Icon name="ClockIcon" size={20} variant="outline" className="text-accent-foreground" />
                                </div>
                                <span className="font-semibold text-foreground">{tx.hoursLabel}</span>
                            </div>
                            <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">{tx.hours}</p>
                        </div>

                        {/* Service areas */}
                        <div className="bg-secondary border border-border rounded-2xl p-6">
                            <p className="text-xs uppercase tracking-widest text-muted-foreground font-semibold mb-4">Service Areas</p>
