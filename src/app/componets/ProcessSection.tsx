...
export default function ProcessSection({ lang }: ProcessSectionProps) {
    ...
    return (
        <section id="process" ref={sectionRef} className="py-24 md:py-32 bg-foreground text-background relative overflow-hidden">
            {/* Decorative large text */}
            <div className="absolute top-8 right-6 md:right-12 pointer-events-none select-none font-display font-bold text-background/5 leading-none"
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-6 md:gap-4">
                    {tx.steps.map((step, i) => (
                        <div
                            key={step.num}
                            className={`relative group cursor-pointer reveal transition-all duration-300 ${activeStep === i ? 'md:scale-105' : ''
                                }`}
                        ...
                        >
                            ...
                            <div className={`relative z-10 p-6 rounded-2xl border transition-all duration-300 ${activeStep === i
                                    ? 'bg-primary border-primary shadow-xl shadow-primary/30'
                                    : 'bg-white/5 border-white/10 hover:bg-white/10 hover:border-white/20'
                                }`}>
                                {/* Number */}
                                <div className="flex items-center justify-between mb-4">
                                    <span className={`text-xs font-mono font-bold tracking-widest ${activeStep === i ? 'text-accent' : 'text-background/30'
                                        }`}>
                                        {step.num}
                                    </span>
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${activeStep === i ? 'bg-white/10' : 'bg-white/5'
                                        }`}>
                                        <Icon
                                ...
                                <h3 className={`text-base font-display font-bold mb-2 ${activeStep === i ? 'text-background' : 'text-background/80'
                                    }`}>
                                    {step.title}
                                </h3>
                                <p className={`text-xs leading-relaxed ${activeStep === i ? 'text-background/80' : 'text-background/40'
                                    }`}>
                                    {step.desc}
                                </p>
                                <div className={`mt-4 flex items-center gap-1.5 text-xs font-medium ${activeStep === i ? 'text-accent' : 'text-background/30'
                                    }`}>
